...
uint32_t WiFiwaitConnect(const uint32_t timeout)
{
    uint32_t start = millis();
    wl_status_t status = WiFi.status();
    while (status != WL_CONNECTED && status != WL_NO_SSID_AVAIL && millis() - start < timeout)
    {
        LOG_INFO(F("Status: ") << (int)status);
        delay(300);
        voltage.update();
        status = WiFi.status();
    }
...
void WiFiConnect()
{
    ...
    {
        LOG_INFO(F("bssid set ") << (int)conf.bssid_set);
        uint32_t timeout = 0;
        bool slow_connect = (conf.bssid_set == 0);
        if (!slow_connect)
        {
            LOG_INFO(F("Fast connect"));
            WiFi.begin();
            timeout = WiFiwaitConnect(ESP_FAST_CONNECT_TIMEOUT);
            wifi_station_get_config(&conf);
        }
        if (conf.bssid_set == 1 && ( timeout > ESP_FAST_CONNECT_TIMEOUT || WiFi.status() == WL_NO_SSID_AVAIL))
        {
            conf.bssid_set = 0;
            wifi_station_set_config(&conf);
            LOG_INFO(F("clear bssid set"));
            slow_connect = true;
        }
        if (slow_connect)
        {
            LOG_INFO(F("Slow connect"));
            char ssid[33], psk[65];
            memcpy(ssid, conf.ssid, sizeof(conf.ssid));
            ssid[32] = 0;
            memcpy(psk, conf.password, sizeof(conf.password));
            psk[64] = 0;
            WiFi.begin(&ssid[0], &psk[0]);
            timeout = WiFiwaitConnect(ESP_CONNECT_TIMEOUT);
        }
        if (!conf.bssid_set)
        {
            if (WiFi.status() == WL_CONNECTED)
            {
                ...
                psk[64] = 0;
                uint8_t bssid[6];
                memcpy(bssid, conf.bssid, sizeof(conf.bssid));
                int32_t channel = WiFi.channel();
                // WiFi.disconnect();
                // LOG_INFO(F("Reconnect"));
                LOG_INFO(F("Safe connect parameter"));
                WiFi.persistent(true);
                res = WiFi.begin(&ssid[0], &psk[0], channel, reinterpret_cast<const uint8_t *>(&bssid), false);
